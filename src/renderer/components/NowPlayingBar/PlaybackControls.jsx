...
export function PlaybackControls({ onTogglePlay, onNext, onPrev, onToggleShuffle, onToggleRepeat, loading }) {
  ...
  return (
    <div className="np-buttons">
      <button className={`icon-btn${shuf ? ' active' : ''}`} onClick={onToggleShuffle} title="Shuffle">
      ...
      <button className={`play-btn${loading ? ' loading' : ''}`} onClick={onTogglePlay} title={loading ? 'Loading...' : playing ? 'Pause' : 'Play'}>
        {loading ? (
      ...
      <button className={`icon-btn${rep !== 'off' ? ' active' : ''}`} onClick={onToggleRepeat} title={rep === 'one' ? 'Repeat One' : rep === 'all' ? 'Repeat All' : 'Repeat'}>
        {repeatIcon}
      </button>
    </div>
  );
